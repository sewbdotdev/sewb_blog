...
import { useGetUserProfileQuery, useUpdateMeMutation } from '@customTypes/generated/graphql';
...
const ProfilePage: NextPage = (props) => {
    ...
    const [bio, setBio] = useState(data?.usersPermissionsUser?.data?.attributes?.bio ?? '');
    ...
    const fill = () => {
        setTwitterUrl(data?.usersPermissionsUser?.data?.attributes?.twitterUrl ?? '');
        setLinkedinUrl(data?.usersPermissionsUser?.data?.attributes?.linkedinUrl ?? '');
                            ...
                            {Boolean(data?.usersPermissionsUser?.data?.attributes?.avatar?.data) ? (
                                <img
                                    ...
                                    src={Helpers.getImageURL(
                                        String(
                                            data?.usersPermissionsUser?.data?.attributes?.avatar
                                                ?.data?.attributes?.url
                                        )
                                    )}
                                    alt={
